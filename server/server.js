...
io.on('connection', (socket) => {
    ...
    socket.on('join', function (params, callback) {
        if (!isRealString(params.name) || !isRealString(params.room)) {
            return callback('Username and/or Name Room requiered.');
        }
        ...
        socket.emit('newMessage', generateMessage('Admin', 'Welcome loser'));
    ...
    socket.on('createMessage', (message, callback) => {
        console.log(message);      
        io.to(socket.params.room).emit('newMessage', generateMessage(message.from, message.text));  
        callback();
    });
